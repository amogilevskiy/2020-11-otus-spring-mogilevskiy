...
public class GenreCommands {
    ...
    @ShellMethod(value = "Delete an existing genre by id", key = {"dg", "delete-genre"})
    public String deleteGenre(@ShellOption String id) {
        if (genreService.deleteById(id)) {
            return localizationService.localize("success.genreDeleted");
        } else {
            return localizationService.localize("error.genreNotDeleted");
        }
    }
